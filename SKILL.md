---
name: taxue-halftone
description: "半调海报：用限墨+网点把任意主题做成有结构、有质感、可复用的艺术封面。支持多种印刷语言（半调/孔版/双色/木刻/蓝晒/复印机、1-bit 剪影），按风格给出一致又有特色的封面。六项不变式出图前先定：纸色、墨色角色、叠印方式、印刷机制、主体走向、字图音量。触发词：半调海报、艺术封面、网点、孔版、双色、Riso、duotone、木刻、蓝晒、复印机美学、平台封面（B站/小红书/抖音/公众号/头像/超宽）、主题结构印刷封面、限墨封面。不用于：全彩实拍、UI、图表、模板拼贴。"
---
# 半调海报 · 艺术封面
...
| 2 | **墨（Ink/Mist）** | 哪几支墨、各干什么 | `roles.json` | 主 70–85%，辅/雾且各 1 职务 |
...
未指定时的默认：机制 AM、双色、中性白纸、relaxed（旅行/生活）或 balanced（活动/信息）、空纸 25–55%、当代编辑（不做旧）、主墨 45°、双色差 ≥30°。Mist 不是第三色相，是主墨浅阶或叠印密度。
...
出图前跑 `scripts/validate_styles.py <style>`，出图后逐项自检（失败只重生一次，只改一项）：
- 超过两墨 / 无选定机制痕迹 / 空纸越界 / 主体不可辨（遮标题 <3 秒认出）/ 无 5× 字号差 / 乱码或虚构品牌 / 自动复古 / 无焦点事件 / 字图同网碎字 / 两版同角 / 结构边被网融化 / 模糊渐变冒充网点
- 答案判语：能否说出「哪支墨、哪个角或哪一层、纸在哪」→ 说不出口即未过关
...
第三色相、渐变、霓虹、全彩照片、矢量平涂网红海报、居中模板、贴纸拼贴、自动泛黄做旧、销售 CTA、Logo/URL/二维码加网、复刻任何参考图独特字句与排版、无来源的完整广告英雄人像、RGB 上套 Color Halftone、全通道同角、假 CMYK 冒充 Riso、把 Riso（印刷机）和 Halftone（加网）当可互换滤镜名、复古电影海报拼盘、把印刷机制当情绪词直接写进 prompt。
