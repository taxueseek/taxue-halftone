...
- 波普波点 polka 与物影蓝晒 aizome_photogram 首次进入发布线：两风格 1.4.0 时已在真源开发完成（含新机制 polka_field、新行为 field），未随 1.2.0–1.4.1 发布；本次双线合并随 1.4.2 首发。
- 真源与发布仓内容对齐：README 中英风格表补齐至十二种风格 + 两个变体、示例区 19 张、评测用例 14 条。

### Changed

- 风格命名统一为自然语言（style_id 不变）：波兰海报路→波兰海报风格、瑞士海报路→瑞士海报风格、孔版 zine 路→孔版 zine 风格、胶片叠层路→胶片叠层风格、标本 / 档案路→标本 / 档案风格、点阵解构海报路→点阵解构海报风格、中式剪纸路→中式剪纸风格、点阵剪纸路→点阵剪纸风格、蓝染艺术海报路→蓝染艺术海报风格、物影蓝晒路→物影蓝晒风格、波普波点路→波普波点风格、经典致敬路→经典致敬风格。同步范围：SKILL.md 风格清单与正文表述（「快速路」改为「精简流程」）、styles name 字段、术语反查（vocabulary）、README、CHANGELOG 历史条目；全部色卡重渲。
...
- 蓝染变体 aizome_photogram（物影蓝晒风格）：反俗套蓝染，蓝底白形的蓝晒物影（photogram 正负反转，母版白底蓝形、变体蓝底白形），现代潮流物件（滑板/耳机/球鞋/自行车零件）平放曝光留白，水洗晕染与颗粒只在蓝区与边缘、白色剪影绝对干净；主墨靛蓝 #1E3A64 大面积 + 钴蓝 #2148B8 一件事务，knockout、still、image_lead、空纸 25–40%；avoid 写满民俗俗套清单（江南水乡、油纸伞、荷花锦鲤、蓝印花布纹、中式窗棂、书法印章）；内置精炼元提示词模板（物件必填，687 字）。
- 第十二套风格 polka（波普波点风格）：波普单元场（design-system 新增机制 polka_field，机制内三选一：波点消融/无限细网/镜屋延展）+ 无中心场行为（新增 field）；同一种视觉单元全幅连续生成、单元吞没主体并漫出边缘、无单一视觉中心；底色/主色/点色 ≤3 色 HEX、丙烯珐琅厚实平涂、边缘锐利；背景图形由主题中文首字或英文首字母构成（如「庄」或 Z），图形内部由波点/细网单元组成；文案中英双语：中文短句为主、英文更小字号呼应（语义呼应非逐字直译）；附 6 条出图自检清单（checklist 字段）；内置精炼元提示词模板（主体必填，主题/首字图形/双语文案可留空自动创作，机制默认波点消融）。
...
- 第十一套风格 tribute（经典致敬风格）：任意名画/艺术流派/年代美学为视觉母题，仅抽象转译构图母题、视觉动机与情绪氛围，不复刻任何可辨认画面；巨大粗体英文标题 × 极小形象的纪念碑式尺度反差，中文短句为主 + 英文短句更小字号呼应（语义呼应非逐字直译）；双色叠印加深、未涂布纸、平版细 AM 半调、轻微褪色油墨与轻微错版；内置元提示词模板，风格来源与主题为必填。
- 第十套风格 aizome（蓝染艺术海报风格）：绝对纯白数字平面（#FFFFFF，design-system 新增 ground_pure_white）+ 深靛蓝 #1E3A64 + 钴蓝 #2148B8 叠印出多次浸染的最深蓝；机制 photogram_stencil（蓝晒式 2-3 明度平阶剪影，不用半调网点），行为 dissipate（染液向外晕散），字图音量 balanced；染液浓淡、颗粒、短排线与毛边只存在于蓝色图形内部，禁止自动做旧与独立悬浮线条。
...
- 第八套风格 dot_dissolve（点阵解构海报风格）：复古丝网圆点阵，具象主体解构为点阵、颗粒与流动波纹，粗体字融入点阵场，标点极简（正文与标题仅逗号句号）；内置元提示词模板与槽位填充规则，主题为唯一必填。
- 第九套风格 paper_cut_group（中式剪纸风格）：剪纸剪影群像层层交错，细节全靠白色负空间镂空，一枚跳色例外成员，上图下卡；内置元提示词模板与槽位推演规则，主题为唯一必填。
...
- 风格更名（style_id 不变）：中式剪纸群像路→中式剪纸风格，网点剪纸群像路→点阵剪纸风格——后者定性为点阵解构与中式剪纸的混搭**变体**（非独立第十风格），旧名未体现；README（中英）、SKILL.md、evals、术语反查、DESIGN.md、色卡、入口桥同步。
...
- 六套风格：editorial（复古编辑印刷风）、polish（波兰海报风格）、swiss（瑞士海报风格）、riso_zine（孔版 zine 风格）、filmstack（胶片叠层风格）、archival（标本档案风格）。
